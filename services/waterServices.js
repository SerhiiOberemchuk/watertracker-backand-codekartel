...
export const getWaterMonth = async (user, date) => {
  ...
  const oneMonthData = await Water.find({ user, date: { $gte: firstDayOfMonth, $lte: lastDayOfMonth } }).lean()

  const values = [];

  for (let day = 1; day <= daysInMonth; day++) {
    const oneDayData = oneMonthData.filter(item => dayjs(item.date).date() === day);
    const date = `${day}, ${currMonth}`;
    const dailyNorm = Math.max(...oneDayData.map(item => item.dailyNorm));
    const sumOfValues = oneDayData.reduce((accumulator, item) => accumulator + item.value, 0);
